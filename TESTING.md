...
- ✅ 게스트 로그인 → 팀 목록(빈 상태) 진입 (`smoke.spec.ts`)
- ✅ 게스트 로그인 → 팀 생성 → 목록에 반영 (`smoke.spec.ts`)
- ✅ (시드 인증) 스쿼드에서 선수 추가 → 반영 (`team-management.spec.ts`)
- ✅ (시드 인증) 일정에 경기 추가 → 반영 (`team-management.spec.ts`)
- ⬜ 후속: 경기 상세 → 쿼터/기록 입력 → 통계 화면 검증

> 모킹은 `mockApi(page, seed?)`로 팀/선수/경기 컬렉션을 인메모리 관리한다. 인증이 필요한
> 화면은 `page.addInitScript`로 localStorage 세션을 심어 곧장 진입한다(로그인 플로우는
> 스모크에서 별도 검증). 셀렉터 주의: Playwright `name`은 부분 일치라 모달의 `추가` 버튼은
> 툴바의 `… 추가`와 겹치므로 `{ exact: true }`로 좁힌다.
